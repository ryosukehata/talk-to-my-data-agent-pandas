...
class SchemaTableConfigManager:
    ...
    def __init__(self, local: bool = True):#"database_description.csv"):
        ...
        if local and os.getenv("DATABASE_DESCRIPTION_PATH") is not None:
            self.db_description = pd.read_csv(os.path.join(Path(__file__).resolve().parent.parent.absolute(),
                                                           os.getenv("DATABASE_DESCRIPTION_PATH")))
        else:
            db_description = DatabaseDescription()
            if db_description.id:
                try:
                    self.db_description = download_registry_dataset_as_dataframe(db_description.id)
    ...
    def load_all_descriptions(self) -> Tuple[Dict[str, str], Dict[str, str]]:
        ...
        try:
            ...
            if missing_columns:
                raise ValueError(f"CSV file {self.csv_path} is missing required columns: {missing_columns}")
            
            # Extract schema descriptions (remove duplicates, keep first occurrence)
            schema_df = df[['schema_name', 'schema_description']].dropna().drop_duplicates('schema_name')
            schema_dict = dict(zip(schema_df['schema_name'].str.strip(), 
                                 schema_df['schema_description'].str.strip()))
            
            # Extract table descriptions (if columns exist)

            table_df = df[['table_name', 'table_description']].dropna().drop_duplicates('table_name')
            table_dict = dict(zip(table_df['table_name'].str.strip(), 
