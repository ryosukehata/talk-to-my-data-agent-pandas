...
export const DataSourceToggle: React.FC<IDataSourceToggleProps> = ({
  ...
  const dataByDataSource = useMemo(() => {
    return multipleMetadata?.reduce((acc, { name, metadata }) => {
      ...
      if (
        currentValue.startsWith(EXTERNAL_DATA_STORE_PREFIX) &&
        currentValue === data_source
      ) {
        acc.push(name);
      }
  ...
  return (
    ...
    >
      {getTooltip()}
      {allowedDataSources &&
        allowedDataSources.includes(DATA_SOURCES.DATABASE) && (
          <ToggleGroupItem value={DATA_SOURCES.DATABASE} className="body">
            <div className="m-2">{t("Database")}</div>
          </ToggleGroupItem>
        )}
      {allowedDataSources &&
        allowedDataSources.includes(DATA_SOURCES.REMOTE_CATALOG) && (
