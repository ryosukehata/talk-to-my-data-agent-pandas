      ...
      >
            ...
            <div className="my-1 flex gap-2">
              ...
              <Badge type="outline" className="text-sm leading-tight">
                {metadata?.data_source
                  ? friendlySourceName(metadata.data_source)
                  : t("file")}
              </Badge>
              {isProcessing ? (
                <Badge type="outline" className="text-sm leading-tight">
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden mn-label-large">
          {isProcessing ? (
            ...
            <ScrollArea
              className={cn("mt-4 min-h-0 flex-1", !fullHeight && "h-96")}
            >
              {viewMode === DATA_TABS.DESCRIPTION ? (
                <div
                  className={cn(!fullHeight && "max-h-[360px] overflow-auto")}
                >
                  <DictionaryTable
                    data={filteredDictionary}
                    searchText={searchText}
