...
import { CustomPromptCreate, CustomPromptResponse, CustomPromptDelete } from './types';
...
export const getCustomPrompts = async (): Promise<{ custom_prompts: CustomPromptResponse[] }> => {
  try {
...
export const getCustomPromptByName = async (name: string): Promise<CustomPromptResponse> => {
  const response = await apiClient.get(`/v1/custom-prompts/${encodeURIComponent(name)}`);
  return response.data;
};
...
export const deleteCustomPrompt = async (
  ...
  const response = await apiClient.delete('/v1/custom-prompts', { data: promptData });
  return response.data;
};

export const getCustomPromptNames = async (): Promise<{ prompt_names: string[] }> => {
