...
describe("UI primitives", () => {
  test("renders clickable badges as buttons and passive badges as divs", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: /passive/i }),
  ...
  test("renders checkbox checked and indeterminate states", () => {
    const { rerender } = render(
      <Checkbox checked aria-label="checked option" />,
    );

    expect(
      screen.getByRole("checkbox", { name: /checked option/i }),
    ).toHaveAttribute("data-state", "checked");

    rerender(<Checkbox checked="indeterminate" aria-label="mixed option" />);
    expect(
      screen.getByRole("checkbox", { name: /mixed option/i }),
