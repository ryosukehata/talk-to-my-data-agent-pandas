...
describe("HighlightText", () => {
  ...
  test("highlights matching text case-insensitively", () => {
    const { container } = render(
      <HighlightText text="Hello World" searchText="world" />,
    );
  ...
  test("escapes special regex characters in search text", () => {
    const { container } = render(
      <HighlightText text="Price: $10.99" searchText="$10.99" />,
    );
  ...
  test("applies custom className", () => {
    render(
      <HighlightText text="test" searchText="" className="custom-class" />,
    );
