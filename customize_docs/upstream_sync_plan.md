...
| `v11.5.1` | `codex/upstream-sync-v11.5.1` | 計画中 | `utils` -> `core/src/core` 移行、`utils/customize` -> `core/src/core/customize` 移行、LiteLLM/LLM構成刷新、Taskfile/CLI中心のDX、workflow分割、infra再配置 | `utils/customize/*` の実装本体は `core.customize` へ移す。旧 `utils.*` / `utils.customize.*` は互換importとして残し、pandas公開挙動は維持する。polars移植なし。 | 詳細は `customize_docs/v11_5_1_integration_plan.md` |
