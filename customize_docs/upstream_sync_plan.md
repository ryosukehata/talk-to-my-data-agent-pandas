...
- 検証: `npm --prefix app_frontend test` 111 passed, `npm --prefix app_frontend run lint` passed with Fast Refresh warnings, `npm --prefix app_frontend run build` passed, `uv run pytest app_backend/tests customize_docs -q` 59 passed / 2 skipped, `app_backend/static` 一時退避時 `56 passed / 5 skipped`, `uv run ruff format --check .` passed, `uv run ruff check .` passed。
