...
- v0.5.3 で追加された Knip CI は、この fork の custom prompts / reports / templates を unused と誤検出するため、`app_frontend/knip.json` に custom 機能用の ignore を追加した。`Reports.tsx` の未使用 default export は実コードから削除した。
