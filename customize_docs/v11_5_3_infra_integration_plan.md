...
- 2026-06-27 dev の Pulumi Up 失敗対応で、旧 workflow の root `requirements.txt` install から `infra` の uv 環境へ移ったことで不足していた `core` 実行時依存を追加した。`core.i18n` が使う `babel`、`core.analyst_db` が使う `langid`、Word生成が使う `python-docx` を `core/pyproject.toml` に追加し、`infra/uv.lock` を更新した。
- 2026-06-27 追加の Pulumi Up 失敗対応で、既存 `APP_ENVIRONMENT_ID` 利用時に `datarobot.ExecutionEnvironment.get(...).id` の Pulumi `Output` を Pydantic `ApplicationSourceArgs` に渡して validation error になる問題を修正した。app backend の `ApplicationSource` args は `pulumi.Input[str]` を直接受ける dict で渡す。
- 2026-06-27 dev の Pulumi Up で `ApplicationSource` 作成後に `CustomApplication` の ready 判定が失敗した。upstream と同じく `app_backend/core -> ../core` symlink をたどって ApplicationSource を作る形に戻し、`core/pyproject.toml` など editable `core` package の metadata も同梱する。併せて `CustomApplication` は `ApplicationSource` に明示依存させ、`required_key_scope_level` を引き継ぐ。
- 2026-06-27 fork 固有の LLM moderation 設定として、旧 `infra/settings_job_infra.py` の `Prompt Tokens` / `Response Tokens` guardrails を `infra/configurations/llm/guardrails.py` に分離した。`CustomModel` を作成する LLM 構成はこのモジュールを import して `guard_configurations` に渡し、upstream 追従対象の本体設定から fork 独自カスタマイズを分ける。
- 2026-06-27 fork 固有の CustomJob / usage monitoring / cleanup job / usage dashboard 定義を `infra/configurations/jobs/custom_jobs.py` に分離した。`infra/infra/app_backend.py` は application source / custom application の upstream 型を維持し、job 関連の Pulumi resource は jobs configuration を import して作成する。
